            ...
            <div class="col-xl-3 col-md-6 mb-4">
              <div class="card border-left-info shadow h-100 py-2">
                <div class="card-body">
                  <div class="row no-gutters align-items-center">
                    <div class="col mr-2">
                      <div class="text-xs font-weight-bold text-info text-uppercase mb-1">Persentasi Karyawan Masuk</div>
                      <div class="row no-gutters align-items-center">
                        <div class="col-auto">
                          <div class="h5 mb-0 mr-3 font-weight-bold text-gray-800">
                            <?php
                            $total_karyawan = mysqli_num_rows(mysqli_query($link, "SELECT * FROM data_karyawan"));
                            $hari_ini = date('Y-m-d');
                            $total_karyawan_masuk_hari_ini = mysqli_num_rows(mysqli_query($link, "SELECT * FROM data_absen WHERE tanggal = '$hari_ini' GROUP BY `uid`"));
                            $persentase_karyawan_masuk = round($total_karyawan_masuk_hari_ini / $total_karyawan * 100);
                            echo "$persentase_karyawan_masuk%";
                            ?>
                          </div>
                        </div>
                        <div class="col">
                          <div class="progress progress-sm mr-2">
                            <div class="progress-bar bg-info" role="progressbar" style="width: <?= $persentase_karyawan_masuk ?>%" aria-valuenow="2" aria-valuemin="0" aria-valuemax="100"></div>
                          </div>
                        </div>
                      </div>
                    </div>
