					...
					<div class="card shadow mb-4">
						...
						<div class="card-body">
							<div class="col-md-12">
								...
								$orderBy = array('uid', 'nama', 'jabatan', 'jenis_kelamin', 'no_hp', 'alamat');
								$order = 'data_karyawan.id';
								...
								$sql = "SELECT *,data_karyawan.id FROM data_karyawan JOIN data_jabatan ON data_jabatan.id = data_karyawan.id_jabatan ORDER BY $order $sort LIMIT $offset, $no_of_records_per_page";
								if (!empty($_GET['search'])) {
									$search = ($_GET['search']);
									$sql = "SELECT * FROM data_karyawan
								WHERE CONCAT (uid,nama,jabatan,no_hp,alamat)
								LIKE '%$search%'
								ORDER BY $order $sort 
								...
								if ($result = mysqli_query($link, $sql)) {
									if (mysqli_num_rows($result) > 0) {
										...
										echo "<th><a href=?search=$search&sort=&order=jenis_kelamin&sort=$sort>Jenis Kelamin</th>";
										echo "<th><a href=?search=$search&sort=&order=no_hp&sort=$sort>Nomer Hp</th>";
